...
public class HexGridChunk : MonoBehaviour
{
    ...
    private void Triangulate(HexDirection direction, HexCell cell)
    {
        ...
        EdgeVertices e = new(center + HexMetrics.GetFirstSolidCorner(direction), center + HexMetrics.GetSecondSolidCorner(direction));
    ...
    private void TriangulateWater(HexDirection direction, HexCell cell, Vector3 center)
    {
        ...
        if (neighbor is not null && !neighbor.IsUnderwater)
            TriangulateWaterShore(direction, cell, neighbor, center);
        else TriangulateOpenWater(direction, cell, neighbor, center);
    }
    ...
    private void TriangulateOpenWater(HexDirection direction, HexCell cell, HexCell neighbor, Vector3 center)
    {
        ...
        if (direction <= HexDirection.SE && neighbor != null)
        {
            ...
            if (direction <= HexDirection.E)
            {
                ...
                if (nextNeighbor is null || !nextNeighbor.IsUnderwater) return;
    ...
    private void TriangulateWaterShore(HexDirection direction, HexCell cell, HexCell neighbor, Vector3 center)
    {
        ...
        if (neighbor.ColumnIndex < cell.ColumnIndex - 1) center2.x += HexMetrics.wrapSize * HexMetrics.innerDiameter;
        else if (neighbor.ColumnIndex > cell.ColumnIndex + 1) center2.x -= HexMetrics.wrapSize * HexMetrics.innerDiameter;
        center2.y = center.y;
        ...
        HexCell nextNeighbor = cell.GetNeighbor(direction.Next());
        if (nextNeighbor is not null)
        {
            Vector3 center3 = nextNeighbor.Position;
            if (nextNeighbor.ColumnIndex < cell.ColumnIndex - 1) center3.x += HexMetrics.wrapSize * HexMetrics.innerDiameter;
            else if (nextNeighbor.ColumnIndex > cell.ColumnIndex + 1) center3.x -= HexMetrics.wrapSize * HexMetrics.innerDiameter;
            ...
            waterShore.AddTriangleUV(new(0f, 0f), new(0f, 1f), new(0f, nextNeighbor.IsUnderwater ? 0f : 1f));
    ...
    private void TriangulateEstuary(EdgeVertices e1, EdgeVertices e2, bool incomingRiver, Vector3 indices)
    {
        ...
        waterShore.AddTriangleUV(new(0f, 1f), new(0f, 0f), new(0f, 0f));
        waterShore.AddTriangleUV(new(0f, 1f), new(0f, 0f), new(0f, 0f));
        ...
        estuaries.AddQuadUV(new(0f, 1f), new(0f, 0f), new(1f, 1f), new Vector2(0f, 0f));
        estuaries.AddTriangleUV(new(0f, 0f), new(1f, 1f), new(1f, 1f));
        estuaries.AddQuadUV(new(0f, 0f), new(0f, 0f), new(1f, 1f), new Vector2(0f, 1f));
        ...
        if (incomingRiver)
        {
            estuaries.AddQuadUV2(new(1.5f, 1f), new(0.7f, 1.15f), new(1f, 0.8f), new Vector2(0.5f, 1.1f));
            estuaries.AddTriangleUV2(new(0.5f, 1.1f), new(1f, 0.8f), new(0f, 0.8f));
            estuaries.AddQuadUV2(new(0.5f, 1.1f), new(0.3f, 1.15f), new(0f, 0.8f), new Vector2(-0.5f, 1f));
        }
        else
        {
            estuaries.AddQuadUV2(new(-0.5f, -0.2f), new(0.3f, -0.35f), new(0f, 0f), new Vector2(0.5f, -0.3f));
            estuaries.AddTriangleUV2(new(0.5f, -0.3f), new(0f, 0f), new(1f, 0f));
            estuaries.AddQuadUV2(new(0.5f, -0.3f), new(0.7f, -0.35f), new(1f, 0f), new Vector2(1.5f, -0.2f));
        }
    }
    ...
    private void TriangulateWithoutRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        if (cell.HasRoads)
        {
            ...
            TriangulateRoad(center, Vector3.Lerp(center, e.v1, interpolators.x), Vector3.Lerp(center, e.v5, interpolators.y), e,
                cell.HasRoadThroughEdge(direction), cell.Index);
        }
    }
    ...
    private Vector2 GetRoadInterpolators(HexDirection direction, HexCell cell)
    {
        ...
        if (cell.HasRoadThroughEdge(direction)) interpolators.x = interpolators.y = 0.5f;
        else
        {
    ...
    private void TriangulateAdjacentToRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        EdgeVertices m = new(Vector3.Lerp(center, e.v1, 0.5f), Vector3.Lerp(center, e.v5, 0.5f));
    ...
    private void TriangulateRoadAdjacentToRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        if (cell.HasRiverBeginOrEnd) roadCenter += HexMetrics.GetSolidEdgeMiddle(cell.RiverBeginOrEndDirection.Opposite()) * (1f / 3f);
    ...
    private void TriangulateWithRiverBeginOrEnd(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        if (!cell.IsUnderwater)
        {
            ...
            if (reversed) rivers.AddTriangleUV(new(0.5f, 0.4f), new(1f, 0.2f), new(0f, 0.2f));
            else rivers.AddTriangleUV(new(0.5f, 0.4f), new(0f, 0.6f), new(1f, 0.6f));
    ...
    private void TriangulateWithRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        EdgeVertices m = new(Vector3.Lerp(centerL, e.v1, 0.5f), Vector3.Lerp(centerR, e.v5, 0.5f), 1f / 6f);
    ...
    private void TriangulateConnection(HexDirection direction, HexCell cell, EdgeVertices e1)
    {
        ...
        else TriangulateEdgeStrip(e1, weights1, cell.Index, e2, weights2, neighbor.Index, hasRoad);
        ...
        if (direction <= HexDirection.E && nextNeighbor is not null)
        {
            ...
            if (cell.Elevation <= neighbor.Elevation)
            {
                ...
                else TriangulateCorner(v5, nextNeighbor, e1.v5, cell, e2.v5, neighbor);
            }
            else if (neighbor.Elevation <= nextNeighbor.Elevation) TriangulateCorner(e2.v5, neighbor, v5, nextNeighbor, e1.v5, cell);
            else TriangulateCorner(v5, nextNeighbor, e1.v5, cell, e2.v5, neighbor);
        }
    }
    ...
    private void TriangulateCorner(Vector3 bottom, HexCell bottomCell, Vector3 left, HexCell leftCell, Vector3 right, HexCell rightCell)
    {
        ...
        if (leftEdgeType == HexEdgeType.Slope)
        {
            ...
            else TriangulateCornerTerracesCliff(bottom, bottomCell, left, leftCell, right, rightCell);
        }
    ...
    private void TriangulateRoad(Vector3 center, Vector3 mL, Vector3 mR, EdgeVertices e, bool hasRoadThroughCellEdge, float index)
    {
        if (hasRoadThroughCellEdge)
        {
            ...
            roads.AddTriangleUV(new(1f, 0f), new(0f, 0f), new(1f, 0f));
            roads.AddTriangleUV(new(1f, 0f), new(1f, 0f), new(0f, 0f));
            roads.AddTriangleCellData(indices, weights1);
            roads.AddTriangleCellData(indices, weights1);
        }
        else TriangulateRoadEdge(center, mL, mR, index);
    }

    private void TriangulateRoadEdge(Vector3 center, Vector3 mL, Vector3 mR, float index)
    {
        roads.AddTriangle(center, mL, mR);
        roads.AddTriangleUV(new(1f, 0f), new(0f, 0f), new(0f, 0f));
