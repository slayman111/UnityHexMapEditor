...
public class HexGrid : MonoBehaviour
{
    ...
    private void CreateCell(int x, int z, int i)
    {
        ...
        if (x > 0)
        {
            ...
            if (wrapping && x == cellCountX - 1) cell.SetNeighbor(HexDirection.E, cells[i - x]);
        }
        if (z > 0)
            if ((z & 1) == 0)
            {
                ...
                else if (wrapping) cell.SetNeighbor(HexDirection.SW, cells[i - 1]);
            }
            else
            {
                cell.SetNeighbor(HexDirection.SW, cells[i - cellCountX]);
                if (x < cellCountX - 1) cell.SetNeighbor(HexDirection.SE, cells[i - cellCountX + 1]);
                else if (wrapping) cell.SetNeighbor(HexDirection.SE, cells[i - cellCountX * 2 + 1]);
            }
    ...
    private bool Search(HexCell fromCell, HexCell toCell, HexUnit unit)
    {
        ...
        while (searchFrontier.Count > 0)
        {
            ...
            for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++)
            {
                ...
                if (neighbor is null || neighbor.SearchPhase > searchFrontierPhase || !neighbor.Explorable) continue;
    ...
    public void CenterMap(float xPosition)
    {
        ...
        for (int i = 0; i < columns.Length; i++)
        {
            if (i < minColumnIndex) position.x = chunkCountX * (HexMetrics.innerDiameter * HexMetrics.chunkSizeX);
            else if (i > maxColumnIndex) position.x = chunkCountX * -(HexMetrics.innerDiameter * HexMetrics.chunkSizeX);
            else position.x = 0f;
